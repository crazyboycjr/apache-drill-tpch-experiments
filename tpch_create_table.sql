...
create or replace  view region as select
    cast(columns[0] as int) r_regionkey,
    cast(columns[1] as char(25)) r_name,
    cast(columns[2] as varchar(152)) r_comment
from dfs.`/tpch-data/region.tbl`;

create or replace  view nation as select 
...
from dfs.`/tpch-data/nation.tbl`;

create or replace  view part as select
...
from dfs.`/tpch-data/part.tbl`;


create or replace  view supplier as select
...
from dfs.`/tpch-data/supplier.tbl`;

create or replace  view partsupp as select
...
from dfs.`/tpch-data/partsupp.tbl`;
...
from dfs.`/tpch-data/customer.tbl`;
...
from dfs.`/tpch-data/lineitem.tbl`;
